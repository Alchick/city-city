...
ALLOWED_EXTENSIONS = set(['pdf', 'jpeg', 'doc', 'docx', 'txt', 'jpg'])
...
def save_request_data(request):
    try:
        ...
        elif regexp(request.form['article_name'], r'[^a-zA-Z\s0-9]'):
            return 'Wrong article_name'
        elif regexp(request.form['author_name'], r'[^a-zA-Z\s0-9]'):
            return('wrong author_name')
        elif not(regexp(request.form['email'], r'[@\w+]')):#do not check. why?
            return('wrong email')
        else:
...
#when table is lock, happened nothing. operations wait their turn. So when it is, operation make
        ...
        if ex.orig.pgcode == '23505':
            return ex.orig.message.split('DETAIL:')[1]
